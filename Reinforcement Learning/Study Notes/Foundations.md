...
3. **Transition Probability ((P(s'|s,a)))** – Probability of moving to state (s') given current state (s) and action (a).
...
* Find a **policy** (\pi(a|s)) that maximizes the **expected cumulative discounted reward**:
...
**Q-function** (Q^\pi(s,a)) is expected return after taking action (a) in state (s) and following (\pi) afterward:
$$Q^\pi(s,a) = \mathbb{E}_\pi \Big[ r_0 + \gamma V^\pi(s_1) \mid s_0=s, a_0=a \Big]$$
...
**Advantage function** measures how much better taking action (a) is compared to the average under policy (\pi):
...
* Combines evaluation and improvement in a **single step** using **Bellman optimality equation**:
